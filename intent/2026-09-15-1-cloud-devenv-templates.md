...
2. **MCP scope:** servers are configured for all four providers by default (decided). Should the agent's default be read-only, with mutating tools opt-in (proposed), or full access?
