...
status: approved
issue: 14
intent: intent/2026-09-16-14-cloud-onboarding.md
---

# Spec: Cloud onboarding and evidence-based reporting
...
- A custom cross-cloud scanner framework: too much infrastructure for one
  automated provider; native tools and shared reporting conventions suffice.
- Prowler/AZQR/OCI scanner integration as mandatory dependencies: useful for
  broader assessment later, but additional installation, permissions, runtime,
  and output normalization are not needed for the initial bounded baseline.
- Automatic remediation, a dashboard service, scheduled jobs, or cloud report
  storage: outside the approved onboarding scope and adds operational risk.
- Automatic skill replacement: could destroy project-specific instructions.
- Treating denied reads as missing resources or reporting a compliance score:
  gives misleading confidence from incomplete evidence.
